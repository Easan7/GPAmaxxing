...
def _generate_branch_response(state: GraphState, branch: str) -> str:
    ...
    branch_directive = {
        ...
        "WEAKNESS": (
            ...
            "Each action item must have a bold main action/verb phrase followed by colon and explanation: '- **Review SourceName**: what to focus on' or '- **Practice X**: rationale'. "
            ...
            "Format as plain bullets without numbering or markdown separators like '---'. "
            "Do not provide full plan structures, minute allocations, or weekly schedules unless PLAN intent is present."
        ),
        "PLAN": (
    ...
    short_prompt = (
        ...
        "If rag_resources is provided, incorporate them as action items within the main next-steps list with bold main actions: '- **Review FileName**: why and what to focus on'. "
        "If rag_mistake_context is provided, prioritize those mistake-to-note links so each resource action maps to a specific wrong question pattern. "
        "For each action item, bold the main action verb/phrase, then colon and explain: '- **Practice topic X**: to reinforce understanding'. "
        "Do not use labels like 'linked query'; describe it naturally as what to review next. "
        "Use markdown bold for section headings where appropriate (for example: **Next Steps**). "
    ...
    long_prompt = (
        ...
        "If rag_resources is provided, integrate them as action items within a unified '**Next Steps**' list, not as a separate section. Connect each resource action to a misunderstanding from recent wrong attempts. "
        "Format each action item with bold main action: '- **Review FileName.pdf**: focus on key concepts' or '- **Practice topic X**: to reinforce understanding'. Bold the action verb/phrase, then colon and explain rationale. "
        "Do not use labels like 'linked query'. Do not output markdown separators like '---' or horizontal rules. "
        f"{branch_directive} "
        "For TREND/WEAKNESS: identify 2-3 core issues, explain why each is happening from mastery/trend/retention-risk/error signals, and provide a unified list of 2-5 concrete next micro-actions (direct actions and/or resource reviews). "

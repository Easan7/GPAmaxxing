...
export default function ChatPage() {
    ...
    const visibleMessages = useMemo(() => {
        ...
        return messages.filter((message) => {
            ...
            const isWelcome = content.includes("ask me anything about your learning progress");
            return !isWelcome;
        });
    }, [messages, planModeEnabled]);
            ...
            <div className="flex-1 flex flex-col min-w-0">
                <div className="h-14 px-6 flex items-center justify-between border-b border-gray-100 bg-white shrink-0">
                    ...
                    <button
                        ...
                        className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold border transition ${planModeEnabled
                                ? "bg-indigo-600 text-white border-indigo-600"
                                : "bg-white text-gray-600 border-gray-300 hover:border-gray-400"
                            }`}
                    >
                        <span
                            className={`h-2 w-2 rounded-full ${planModeEnabled ? "bg-white" : "bg-gray-400"
                                }`}
                        />
                    ...
                    <div className="max-w-3xl mx-auto px-4 py-4">
                        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm flex items-end gap-2 p-3">
                            ...
                            <button
                                onClick={handleSend}
                                disabled={!canSend}
                                className={`h-9 px-4 rounded-xl text-sm font-semibold transition ${canSend
...
function Avatar({ role }) {
    ...
    return (
        <div
            className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${isUser ? "bg-gray-900 text-white" : "bg-gradient-to-br from-pink-500 to-orange-400 text-white"
                }`}
    ...
    return (
        <div className="rounded-2xl border border-indigo-100 bg-indigo-50/60 px-4 py-4 shadow-sm">
            ...
            <div className="mt-3 rounded-lg bg-white border border-indigo-100 px-3 py-3">
                <p className="text-xs font-semibold text-indigo-700 mb-1">Recommended template</p>
                <p className="text-sm text-gray-700">
                    Create a [time horizon] plan, with [total/daily time], focused on [topic scope].
                </p>
            </div>
        </div>
    );
